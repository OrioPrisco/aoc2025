...
fn run(input : File) -> Result<(), Box<dyn Error>> {
    ...
    let mut map = Map::from_lines(input.iter().map(|s| s.as_bytes()));
    println!("{map:?}");
    let mut total = 0;
    loop {
        let count = map.iter().filter(|(_, _, &c)| c == '@' as u8).map(|(x, y, _)|
            (x, y, map.full_around(x,y).filter(|(_, _, &c)| c == '@' as u8).count())
        ).collect::<Vec<_>>();
        let removed = count.iter().filter(|(_,_,count)| *count < 4).count();
        for (x,y,_) in count.iter().filter(|(_,_,count)| *count < 4) {
            *map.at_mut(*x,*y) = '.' as u8;
        }
        if removed == 0 {
            break;
        }
